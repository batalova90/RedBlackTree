...
public:
	...
	private:
		...
		{
			Node<T, T1>* x = nullptr;
			while (current->right != nullptr) //Íåîáõîäèìî äîõîäèòü äî îãðàíè÷èòåëÿ!!!
			{
				x = current;
	...
	{
		out << root.data_ << " äàííûå \t" << root.key_ << " êëþ÷ \t" << std::endl;
		return out;
	}
...
void MultiMap<T, T1>::outputTreeStraight()
{
	if (node_ != nullptr)
		...
		std::cout << "Ïóñòîå äåðåâî!" << std::endl;
}
...
{
	std::cout << "Êëþ÷: " << current->key_ << " \t" << "Äàííûå: " << current->data_ << "\t" << "Öâåò: " << current->color_ << std::endl;
...
void MultiMap<T, T1>::rbDeleteFixUp(Node<T, T1>* x)
{
	...
	{
		if (x == x->p->left) //åñëè ëåâûé ðåáåíîê
		{
			w = x->p->right;
			if (w->color_ == "red")
			{
...
{
	Node<T, T1>* y = nullptr;
	if (current != node_)//åñëè êîðåíü, òî ïðîñòî îêðàø. â ÷
	{
		while (current->p->color_ == "red" and current != node_)

		{
			if (current->p == current->p->p->left) //îòåö ëåâûé ðåáåíîê
			{
				y = current->p->p->right;
